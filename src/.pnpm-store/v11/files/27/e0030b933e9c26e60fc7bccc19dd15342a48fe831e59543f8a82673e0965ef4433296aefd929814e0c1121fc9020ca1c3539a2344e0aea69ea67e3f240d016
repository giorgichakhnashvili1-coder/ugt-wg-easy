...
import { createLocaleConfigs, resolveDefaultLocale, resolveSupportedLocale } from "../shared/locales.js";
import { setupVueI18nOptions } from "../shared/vue-i18n.js";
import { createNuxtI18nContext, isPrerenderable, useLocaleConfigs } from "../context.js";
...
export default defineNuxtPlugin({
  ...
  async setup(_nuxt) {
    ...
    prerenderRoutes(
      localeCodes.filter(isPrerenderable).map((locale) => `${__I18N_SERVER_ROUTE__}/${__I18N_LOCALE_HASHES__[locale]}/${locale}/messages.json`)
    );
    ...
    extendI18n(i18n, {
      extendComposer(composer) {
        ...
        composer.localeProperties = computed(
          () => withRuntimeDomain(
            normalizedLocales.find((l) => l.code === composer.locale.value) || { code: composer.locale.value, domains: [], defaultForDomains: [] },
