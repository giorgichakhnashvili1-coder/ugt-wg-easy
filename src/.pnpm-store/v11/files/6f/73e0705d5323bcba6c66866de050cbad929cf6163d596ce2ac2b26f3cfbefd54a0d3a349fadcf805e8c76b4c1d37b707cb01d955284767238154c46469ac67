...
const getBareHttpUrl = (description, enabled) => {
  ...
  return bareHttpUrlRegex.test(trimmedDescription) && URL.canParse(trimmedDescription) ? trimmedDescription : null;
};
...
const getExpectedMessage = (bareUrl, canonicalForm) => {
  return bareUrl ? 'Expected bare @see URL to use a {@link} tag.' : `Expected @see link to use the ${canonicalForm} form.`;
};
...
var _default = exports.default = (0, _iterateJsdoc.default)(({
  ...
  const descriptionMatcher = canonicalForm === 'pipe' ? new RegExp(`${markdownLinkRegex.source}|${prefixLinkRegex.source}`, 'v') : new RegExp(`${markdownLinkRegex.source}|${pipeLinkRegex.source}`, 'v');
  ...
  for (const tag of jsdoc.tags) {
    ...
    const rawDescription = firstTokens.name ? firstTokens.name + firstTokens.postName + tagDescription : tagDescription;
    ...
    for (const inlineTag of tag.inlineTags) {
      ...
      if (isInsideMarkdownCodeSpan(rawDescription, start) || scopedPackageNameRegex.test(inlineTag.namepathOrURL)) {
        hasAmbiguousLink = true;
      } else {
        hasNoncanonicalLink = true;
      }
    }
    const unrecognizedLinkText = rawDescription.replaceAll(new RegExp(markdownLinkRegex, 'gv'), '').replaceAll(new RegExp(prefixLinkRegex, 'gv'), '').replaceAll(new RegExp(pipeLinkRegex, 'gv'), '');
    hasAmbiguousLink ||= markdownLinkAttemptRegex.test(unrecognizedLinkText) || prefixLinkAttemptRegex.test(unrecognizedLinkText) || pipeLinkAttemptRegex.test(unrecognizedLinkText);
    if (hasAmbiguousLink) {
  ...
  for (const [reportIdx, tag] of fixableTags.entries()) {
    const bareUrl = bareUrlTags.get(tag);
    utils.reportJSDoc(getExpectedMessage(bareUrl, canonicalForm), {
      line: tag.source[0].number
    }, enableFixer && reportIdx === 0 ? () => {
      for (const fixableTag of fixableTags) {
        const firstTokens = fixableTag.source[0].tokens;
        if (firstTokens.name) {
          firstTokens.description = firstTokens.name + firstTokens.postName + firstTokens.description;
          firstTokens.name = '';
        ...
        if (fixableBareUrl) {
          for (const source of fixableTag.source) {
            ...
            source.tokens.description = source.tokens.description.replace(fixableBareUrl, () => {
        ...
        for (let sourceIdx = 0; sourceIdx < fixableTag.source.length; sourceIdx++) {
          if (!descriptionMatcher.test(fixableTag.source[sourceIdx].tokens.description)) {
            continue;
          }
          utils.setTagDescription(fixableTag, descriptionMatcher, description => {
...
//# sourceMappingURL=normalizeSeeLinks.cjs.map